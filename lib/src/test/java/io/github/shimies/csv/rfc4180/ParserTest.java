...
public abstract class ParserTest {
  ...
  protected void testEscapedEmptyFields() throws IOException {
    ...
    String empty = "";
    String record =
        util.toCsvByDuplicatingField(empty, 1, nFields, false) + CsvUtil.escapeRFC4180("");
  ...
  protected void testEscapedFieldsSurroundedBySpaces(UnaryOperator<String> spaceAdder)
      throws IOException {
    int nRecords = 1;
  ...
  protected void testEscapedFieldWithNoClosingDquote() {
    int nRecords = 1;
    String csv =
        util.toCsvByDuplicatingFieldsRecord(Arrays.asList("abc", "\"an escaped field"), nRecords);
    ParserException e = assertThrows(ParserException.class, () -> parser.parseString(csv));
  ...
  protected void testValiadicFields() throws IOException {
    ...
    List<String> records =
        csvData.stream()
            .map(xs -> util.toCsvByDuplicatingFieldsRecord(xs, 1, false))
  ...
  private void testRepeatedFields(String field, boolean mustEscape, int nRecords, int nFields)
    ...
    if (mustEscape) fieldEscaped = CsvUtil.escapeRFC4180(fieldEscaped);
    String csv = util.toCsvByDuplicatingField(fieldEscaped, nRecords, nFields);
    List<List<String>> records = parser.parseString(csv);
    if (stripFields() && !mustEscape) field = field.strip();
