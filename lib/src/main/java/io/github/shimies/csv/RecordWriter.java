 ...
 * implementing this interface must be mutable, and thus may not be thread safe.
 */
public interface RecordWriter {
  ...
  default void writeAllRecords(CsvParser parser, Reader reader) throws IOException {
    ...
    while (r.hasMoreRecord()) writeRecord(r.readRecord());
  }

  default void writeAllRecords(Iterable<List<String>> records) throws IOException {
    for (List<String> record : records) writeRecord(record);
  }
